...
4. **Realistic preview generation approach — resolved in Phase 9.** TRD §17.1 left this intentionally open (diffusion/image-edit pipeline vs. compositing-first + AI enhancement). Decision: diffusion-based photorealistic image generation stays **permanently out of v1 scope** — a real image-generation integration is materially heavier than the text-only AI Provider Router built this phase (a different provider, different cost/latency profile, different failure modes), and CLAUDE.md's guaranteed-baseline contract (deterministic collage) is already fully satisfied without it. `compose_realistic()` now returns one of three distinct, honest reasons for unavailability (user's `local_preferred` choice / no cloud provider configured / permanent scope decision) instead of one generic message, so the UI is truthful about *why* rather than implying "coming soon."
5. **Schema-naming drift from `documents/04`** — discovered while planning Phase 4: the baseline migration (`infra/db/0001_baseline.sql`, Phase 0) was built from summary documents, not the System Design Document's literal DDL, and drifted from it. The doc keys every table off `user_profile_id → user_profiles.id`; every table actually built keys off `user_id → users.id` directly. Several field names also differ (e.g. `wardrobe_items.image_url` vs. the doc's `local_uri`/`cloud_uri`; `users.hashed_password` vs. `password_hash`; `wardrobe_items` is still missing the doc's `subcategory`/`season`/`fit` columns — `formality` was added in Phase 5). Phase 4's new tables (`calendar_events`, `routines`, `weather_cache`), Phase 5's new tables (`outfits`, `recommendation_runs`, `outfit_history`), and Phase 6's new table (`preview_assets`) adopt the doc's exact table/column names but keep the `user_id` FK pattern for consistency with every router's existing ownership-check code. Not retrofitting Phases 0–3 now — tracked here so it stays visible; revisit if/when a schema-alignment pass is scheduled.
6. **Tone value set and chat-assistant backend designed fresh in Phase 7** — `documents/04`'s canonical `user_profiles.tone_preference` column has a default (`'practical'`) but no enumerated value list anywhere in either doc; `practical`/`direct`/`encouraging` were chosen during implementation. No chat/assistant table or endpoint exists in `documents/04` at all — `documents/05` frames "chat interactions" as a client-UI-only concept, so `POST /assistant/refine` and its request/response contract were designed fresh, not extracted. Not a drift from the docs (nothing to drift from) — logged here so it's traceable, same practice as Phase 6's realistic-preview stub.
7. **`RecommendationRun.refined_from_run_id`** — added in Phase 7, beyond the canonical DDL, so a chat-refined run can be traced back to the run it refined (a `RecommendationRun` has no other lineage/versioning field). Same category of justified deviation as `CalendarEvent.corrections` (Phase 4).
8. **`sync_queue` field-naming drift resolved in Phase 8** — the Phase-0 stub migration used `entity_type`/`entity_id`/`operation`/`is_synced`/`synced_at`; Phase 8 is the first time this table was actually wired to a real endpoint (`POST /sync/replay`), so it was brought in line with `documents/04`'s canonical names (`object_type`/`object_id`/`action`/`status`/`retry_count`/`last_error`) rather than perpetuating the stub's drift, matching the precedent already set for `outfits`/`recommendation_runs`/`outfit_history` in Phase 6.
9. **Client-supplied timestamp normalization (Phase 8 bugfix, worth keeping visible)** — every datetime the mobile app sends (`worn_at`, `client_queued_at`, calendar `start_ts`/`end_ts`) arrives as a timezone-aware `Date().toISOString()` string, while every timestamp generated server-side is naive UTC (`datetime.utcnow()`). Comparing the two directly raises `TypeError` in Python and silently corrupts SQLite string comparisons otherwise. Fixed with a shared `schemas._naive_utc` Pydantic validator applied to every client-supplied datetime field. This had been silently breaking the Phase 7 repeat-avoidance window query for any `worn_at` submitted through the real mobile UI (not caught earlier because every prior phase's curl verification constructed naive timestamps by hand) — if a similar bug resurfaces elsewhere, check for a missing `_naive_utc` validator first.
10. **Cloud-credential sandbox limitation (Phase 9)** — no `ANTHROPIC_API_KEY` is available to the backend process in this build/test sandbox, so `ai_provider.generate_cloud_explanation()`'s live happy path (a real Claude response, not just the graceful-degradation branch) has never been exercised end-to-end here. Same category as the Phase 4 Open-Meteo limitation: the integration code is complete, config-gated exactly like every other cloud-optional path in this app, and was verified as far as this sandbox allows — `api.anthropic.com` is reachable (not blocked by the egress proxy, confirmed via a real 401 with a fake key, not a network-policy denial), and every failure mode (no key, bad key, network error) degrades to `cloud_explanation: null` cleanly rather than breaking the guaranteed 1-best-plus-2-alternatives flow. Revisit with real credentials before treating "cloud enhancement" as user-facing-verified rather than code-verified.
